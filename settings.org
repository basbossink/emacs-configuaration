...
#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load
    ...
    (progn
      ...
      (define-key magit-status-mode-map (kbd "j a") '(lambda () (interactive) (start-process "gitk" nil "gitk" "--all")))
      (define-key magit-status-mode-map (kbd "j p") '(lambda () (interactive) (shell-command "rg -g *.csproj \"<PackageReference\\s+Include=\\\"([^^\\\"]+)\\\"\\s+Version=\\\"([^^\\\"]+)\\\"\\s*/>\" -r \"$1 : $2\" --heading"  "*Package References*" "*Package References Errors*")))
      ...
      (define-key magit-status-mode-map (kbd "j f r") '(lambda () (interactive) (start-process "pr" "*Create pull request*" "pr" "develop")))
...
**** Set ispell executable to use to Hunspell
 #+begin_src emacs-lisp :tangle yes
   ...
   (setq ispell-local-dictionary-alist
         '(("en_US" "[[:alpha:]]" "[^[:alpha:]]" "[']" nil ("-d" "en_US" "-i" "utf-8") nil utf-8)))
 #+end_src
...
#+begin_src emacs-lisp :tangle yes
  (defun my-browse-url-portable-chrome (url &rest ARGS)
    ...
    (call-process "C:/Program Files (x86)/Google/Chrome/Application/chrome.exe" nil nil nil url))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (require 'evil)
  (add-to-list 'evil-emacs-state-modes 'mingus-playlist)
  (add-to-list 'evil-emacs-state-modes 'mingus-browse)
#+END_SRC
** Enable Evil
#+BEGIN_SRC emacs-lisp :tangle yes
  (evil-mode 1)
...
(require 'ssh-agency)
(setq ssh-agency-bin-dir "C:/Windows/System32/OpenSSH")
(setq ssh-agency-add-executable "C:/Windows/System32/OpenSSH/ssh-add.exe")
(setq ssh-agency-agent-executable "C:/Windows/System32/OpenSSH/ssh-agent.exe")
#+end_src
** Log parameters
...
#+begin_src emacs-lisp :tangle yes
  ...
  (require 'julia-mode)
#+end_src
*** Start Language server for Julia mode
#+begin_src emacs-lisp :tangle yes
